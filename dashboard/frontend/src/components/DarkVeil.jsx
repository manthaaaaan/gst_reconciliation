...
export default function DarkVeil({
  ...
  useEffect(() => {
    ...
    // Create a new canvas element dynamically per lifecycle mount
    ...
    try {
      ...
      const handleResize = () => {
        ...
        renderer.setSize(Math.max(1, w * resolutionScale), Math.max(1, h * resolutionScale));
      ...
      const renderLoop = () => {
        ...
        try {
          ...
          console.warn('DarkVeil render frame error:', renderErr);
        }
      };
      ...
      console.warn('Could not initialize DarkVeil WebGL:', err);
      if (container.contains(canvas)) {
        container.removeChild(canvas);
      }
    }
...
`;
if (typeof document !== "undefined" && !document.getElementById("kx-bg-darkveil")) {
  const __kxStyle = document.createElement("style");
  __kxStyle.id = "kx-bg-darkveil";
